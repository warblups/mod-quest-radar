...
(`QUEST_WATCH_UPDATE` event), without waiting for the next server sync.

⚠️ Earlier revisions registered `QUEST_WATCH_LIST_CHANGED` here, plus
`QUEST_TURNED_IN` and `QUEST_REMOVED`. **None of those three events exists on
a genuine 3.3.5a client** — they arrived with Cataclysm, and none of the three
strings is present in `wow.exe` 12340. Registering an unknown event raises,
which aborted the rest of `Core.lua` without a visible message (`scriptErrors`
defaults to `0` on this client).
